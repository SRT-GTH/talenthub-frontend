...
| --------------------------------- | ----------------------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OnboardingRightPanel`            | `src/components/shared/OnboardingRightPanel.jsx`                        | `4704:50608` / `2858:23709`     | Shell for all talent auth + onboarding right panels. Provides: bg, TR/BL CSS-blur ellipse glows, grid texture, sparkle stars, snow pattern. Foreground swapped per screen via `panelContent`. Props: `bgColor` (default `#387440`), `ellipseTRColor` (default `#F7EFDD`), `ellipseBLColor` (default `#F9EBEA`), `showGrid` (bool, default `true`), `showSparkleStars` (bool, default `true`), `showSnow` (bool, default `true`), `panelContent` (ReactNode), `toast` (ReactNode — also controls `aria-hidden`). Ellipses are CSS `rounded-full` divs with `filter: blur()` — color prop drives `backgroundColor` directly.                                   |
| `PhotoCard`                       | `src/components/sections/talentAuth/PhotoCard.jsx`                      | various talent panels           | Shared bordered+rotated photo card with corner ellipse glow. Props: `src`, `borderColor`, `rotation`, `left`, `top` or `bottom`, `ellipseSrc`, `cardWidth` (clamp string), `inset` (CSS inset shorthand), `cornerDecoration` (ReactNode, rendered inside rotation wrapper → rotates with card, use for sparkle at corner), `children` (ReactNode, rendered after card with `position:absolute` relative to the group's top-left — express as % of `cardWidth` so they scale with the card). Corner ellipse is always inside the photo container (clipped to rounded border). Outer group has no `overflow:hidden` so children can extend beyond card bounds. |
| `TalentLoginPanelContent`         | `src/components/sections/talentAuth/TalentLoginPanelContent.jsx`        | `4704:50608`                    | Foreground composition for the login right panel. Renders inside an `absolute inset-0` container: 3 `PhotoCard`s (photo1/2/3 at Figma positions), Data Protected chip, GDPR badge, Welcome Back stats card. Overlay chips remain at panel level (panel-level % positioning keeps them in formation). Imports `PhotoCard` from `./PhotoCard.jsx`.                                                                                                                                                                                                                                                                                                             |
| `TalentWelcomePanelContent`       | `src/components/sections/talentAuth/TalentWelcomePanelContent.jsx`      | `2858:23709`                    | Foreground composition for the onboarding welcome right panel. ONE `absolute inset-0` container holds the main photo card AND all 5 overlay cards so they always stay in formation. Props: `showToast` (bool). Overlay cards: Jobs Available, Saved pill, Verified profile, My Experience, Accra Girls SHS institution card.                                                                                                                                                                                                                                                                                                                                 |
| `TalentDobPanelContent`           | `src/components/sections/talentAuth/TalentDobPanelContent.jsx`          | `2236:901`                      | Foreground for the Date-of-Birth step right panel (talent onboarding step 1, 739×916 px). 3 `PhotoCard`s + overlays grouped with their card (Data Protected inside top-left card; GDPA badge + Adult/Youth card inside centre card). Sparkle at panel level. Uses `WatchTutorial` component (bottom-right). Props: none.                                                                                                                                                                                                                                                                                                                                     |
| `TalentPersonalInfoPanelContent`  | `src/components/sections/talentAuth/TalentPersonalInfoPanelContent.jsx` | `2329:3963`                     | Foreground for the "Build Your Profile" step right panel (talent onboarding step 2, 739×916 px). Same `OnboardingRightPanel` shell. Differences vs DOB: centre card 312 px (CARD1_W clamp(125px,17.7vw,312px)), top-left rotation -8°, bottom-left rotation +4.74°, no corner decoration on bottom-left. Overlay grouping identical pattern. Two hand-drawn arrow SVG decorations. Sparkle-26 at panel top-centre. Props: none.                                                                                                                                                                                                                              |
...
| Prop                     | Type                                                                                        | Default     | Notes                                                                                                                                                                            |
| ------------------------ | ------------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `variant`                | `'primary' \| 'secondary' \| 'tertiary' \| 'tertiary-subtle' \| 'tertiary-cream' \| 'icon'` | `'primary'` | Visual treatment per Figma section. `'icon'` is a separate code path (see below) — bypasses the shared text-button chrome entirely.                                              |
| `size`                   | `'lg' \| 'md' \| 'sm'` (`'icon'` variant: `'md' \| 'sm'` only)                              | `'lg'`      | Padding + border-radius preset.                                                                                                                                                  |
...
**`icon` variant `✅ VERIFIED` (2026-07-24)** — added for the Career Buddy chat input bar (Figma `5132:43389` default state, `5132:45079` hover state). A circular icon-only button with no border, shelf, or text chrome — takes an early-return path in `Button.jsx` (`ICON_BASE_CLASSES` / `ICON_SIZE_CLASSES` / `ICON_VARIANT_CLASSES`, all separate from the text-button constants, so no other variant's classes changed). Pass the icon as `children` directly (no `leftIcon`/`rightIcon` wrapping for this variant).

| Property   | Default                          | Hover                                                                     |
| ---------- | -------------------------------- | ------------------------------------------------------------------------- |
| Background | `#f0f0f0`                        | `linear-gradient(135deg, rgba(20,41,22,0.9) 0%, rgba(42,87,48,0.9) 100%)` |
| Icon/text  | `text-content-secondary` (grey)  | `white`                                                                   |
| Size `md`  | `size-10 p-[10px]` (40px circle) | —                                                                         |
| Size `sm`  | `size-8 p-2` (32px circle)       | —                                                                         |

Used by: `ChatThread.jsx` — attach / mic / voice-notes buttons in the Career Buddy input bar.

**`chip` variant `✅ VERIFIED` (2026-07-28)** — added for the ChatThread suggested-reply pills (Figma `5132:43427` default, `5132:45078` hover). Pill-shaped (`rounded-[16px]`), thin uniform border, regular-weight text — none of which fit the text-button `BASE_CLASSES` shelf/bold-border assumptions, so it's an isolated early-return (`CHIP_BASE_CLASSES` / `CHIP_SIZE_CLASSES` / `CHIP_VARIANT_CLASSES`) same as `icon`.

| Property  | Default              | Hover                                                                         |
| --------- | -------------------- | ----------------------------------------------------------------------------- |
| Border    | `#e5e7eb`            | transparent                                                                   |
| Text      | `#575755`            | `#ebf1ec`                                                                     |
| Bg        | white                | `linear-gradient(173.211deg, rgba(20,41,22,0.9) 0%, rgba(42,87,48,0.9) 100%)` |
| Size `md` | `px-[16px] py-[8px]` | —                                                                             |

Used by: `ChatThread.jsx` — welcome/FAQ suggested-reply chips.

**`ghost-icon` variant `✅ VERIFIED` (2026-07-29)** — added for the `EngagementTopBar` stage-trail overflow scroll chevrons (Figma `5132:44983`). Bare icon-only button with NO background/border chrome at all (distinct from `icon`'s filled `#f0f0f0` circle) — just a colour shift on hover/active. Default `text-neutral-dark`, hover/active `#737373` (Figma's documented Neutrals/White/Dark active colour). Sizes: `md` = `size-9 p-2`, `sm` = `size-7 p-1.5`.

Used by: `EngagementTopBar.jsx` — left/right chevrons that appear only on the edge with hidden overflow, paired with a `mask-image` fade on that same edge.
...
| `Toast`                       | `src/components/ui/Toast.jsx`                       | Portal notification system (`Toast`, `ToastItem`, `ToastContainer`). `compact` prop renders the single-line banner (success/error/warning — official 22×22 icons in `assets.jsx` `Toast*Icon`: success `#387440`, error `#C0392B`, warning `#F59638`; exact bg/border-bottom/drop-shadow per variant, title+body `#404040`). `welcome` variant (Figma `5624:67780`): pale green `#f4faf5` / border `#2e7d32`, person-available icon, stacked title/body |
...
## Career Buddy (AI Engagement) `✅ VERIFIED` (2026-07-29)

| Component                  | Path                                                                  | Figma source                                                                                                                                     | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ChatThread`               | `src/components/ui/ChatThread.jsx`                                    | `5132:43308-46274`, `5132:57368` (mode-picker)                                                                                                   | Reusable chat primitive. Bot/user bubbles, suggested-reply chips, message actions (thumbs/retry), edit/version pagination, file-attachment bubble, mic→recording→send input morph, New Chat/History chrome row (restyled 2026-07-29 to Figma `5132:47988`/`47992` — translucent `bg-black/[0.21]` pill, `rounded-[12px]`, white 15px text, 20px icon). `message.auto` (Figma `5132:47342`/`47617`) renders a system-generated confirmation bubble — right-aligned like a user message, a small `[Auto]` label line, copy+edit actions (no thumbs/retry, no fake version pagination). `message.options` (Figma `5132:58211`) renders a 2-col lettered-answer grid inside a bot bubble (MCQ). `message.modeCards` (Figma `5132:57368`, 2026-07-30) renders icon+title+description picker cards — NOT inside the message bubble; `ChatThread` reads the trailing message's `modeCards` and renders them in its own bottom section, sharing ONE `bg-white rounded-[16px] p-[20px] gap-[20px]` box with the input bar (input placeholder swaps to "Curious about any of these?", border becomes `border-t` instead of a full box border) — verified via Figma's `get_metadata`, not just its screenshot, after a first pass wrongly nested the cards inside the bot's text bubble. No knowledge of Career Buddy specifically — reusable for future recruiter/invitee chat surfaces. NB: distinct from the older, simpler `src/components/ui/ChatBubble.jsx` (a promo-card speech-bubble-with-tail from Figma `3384:82014`) — internal bubble renamed `MessageBubble` to avoid confusion between the two.                                                                                                                                           |
| `CareerBuddySection`       | `src/components/sections/profileFilling/CareerBuddySection.jsx`       | `4830:169704-194488`, `5132:43308-46274`, `5132:46553-47551`, `5132:55364-57377`                                                                 | Page composition + FSM state machine. 11-stage breadcrumb (via `EngagementTopBar` non-interactive mode) + `ChatThread` + `TalentProfilePanel`. Route: `/profile/filling/career-buddy`. Real save flow generalized across stages (`STAGE_SAVE_META`/`STAGE_QA_NODE_PREFIX`/`STAGE_TRAIL_INDEX`, 2026-07-30 — originally Educational-Background-only, 2026-07-29): reaching a stage's confirm node sets status `'awaiting-review'` (data collected, not yet saved), distinct from `'done'` which only happens once the panel's own Confirm click succeeds. `handleConfirmStage` simulates one async round-trip per stage: the FIRST attempt always errors (`Error, No internet connection` toast), the retry always succeeds (`Success, {label}` toast + an auto-confirmation chat bubble), then chains into `STAGE_SAVE_META[stageId].nextNodeId` if set (`'post-personality-prompt'` for `personality` as of 2026-08-03 — was `null` pending verified content; `null` for `work-experience`, the current end of the built chain). `MODE_CARD_ICONS` + `enrichedMessages` (renamed from `messagesWithModeCardIcons` 2026-07-30, now also wires the `games-intro` message's `linkButton.onClick`) resolve each `modeCards` entry's icon component and the Game Store link's open-handler at render time (the script file itself stays icon/callback-free). Owns `gameStoreOpen` state and renders `GameStoreModal`; `handlePlayGameStub` shows a stub toast for "Let's Play" since gameplay isn't built yet. `handleConfirmStage`'s second param (`fieldsOrEntries`) branches on whether `STAGE_FIELD_DATA[stageId]` uses `entries` (Work Experience) or flat `fields` (every other stage) when writing the confirmed value back onto `stages`. |
| `GameStoreModal`           | `src/components/sections/profileFilling/GameStoreModal.jsx`           | `5132:60815` (Game Store), `5132:61301` (Game details)                                                                                           | Games-mode catalog + detail modal (2026-07-30), reuses `ui/Modal.jsx` for the backdrop/portal/ESC-close/scroll-lock. Internal `view` state ('grid'\|'details') switches between the search+category-filter+7-card grid and a selected game's About/traits/how-to-play page, matching Figma's in-modal "← Back to Game Store" nav. Game data lives in `gameStoreData.js` (only "The Escape Room" has real detail-page copy — others render card-level fields only, no fabricated content). Per-game art is an initial-letter placeholder pending real illustrated thumbnails. Gameplay itself (4-stage Escape Room) is stubbed via a `warning` toast on "Let's Play →".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `JobPostFormModal`         | `src/components/sections/profileFilling/JobPostFormModal.jsx`         | `5132:69702` / `70013`–`71289` / upload `75730`                                                                                                  | Shared job form. `mode`: `manual` \| `upload` \| `conversation` switches Figma header/subtitle. Controlled `value`/`onChange` draft; live Jobs panel via parent. `onRequestPost` → confirm. Data in `jobPostFormData.js`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ConfirmJobPostModal`      | `src/components/sections/profileFilling/ConfirmJobPostModal.jsx`      | `5132:72577` / `72890`                                                                                                                           | Nested confirm after Post Job / Jobs panel Confirm / file-upload review. Success → toast + `job-posted-success` (`5132:72901`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `WardAccountSetupModal`    | `src/components/sections/profileFilling/WardAccountSetupModal.jsx`    | `5132:80134` / `5132:80489` / success `5132:80762`                                                                                               | Parent ward account setup when not initiated. Steps: `details` (form + Ghana Card) → `password` (Create / I'll do this later). Data: `wardAccountSetupData.js`. Wired from Guide chip + DemoNavigator `?cb=ward-*`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `jobConversationData`      | `src/components/sections/profileFilling/jobConversationData.js`       | `5132:73294` / `73460`                                                                                                                           | Conversation-with-AI copy + `applyJobConvAnswer` / `buildJobsPanelFields`. Unmapped form fields stay empty.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `RecruiterPanel`           | `src/components/sections/profileFilling/RecruiterPanel.jsx`           | `5132:65996` / `66804` / Jobs reuse                                                                                                              | Company Info. + Jobs rows. Jobs: live preview from chat; Modify / View full form → JobPostFormModal; Confirm → ConfirmJobPostModal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `TalentProfilePanel`       | `src/components/sections/profileFilling/TalentProfilePanel.jsx`       | `4830:169704-194488`, `5132:40348`, `5132:47002` (view-details), `5132:47182` (edit-details), `5132:50598`/`50994` (Work Experience multi-entry) | Right-panel checklist. Expand/Modify/Confirm per row; `fields` (confirmed, live-collected) vs `previewFields` (dimmed, no actions — static reference data for sections not yet chat-collected) vs `entries` (2026-08-03, Work Experience only; **2026-08-07 layout overflow fix** vs Figma 5132:50598 — titles wrap, chevron accordion, value flex-1 min-w-0 break-words, card #e8e8e8/#f8f8f8 — a LIST of collapsible job sub-cards instead of a flat field list, since Figma shows multiple job entries are supported; only entries with real Q&A-collected `fields` are expandable, checked via a single `expandedEntryId` accordion state). Row styling re-verified 2026-07-29 (was wrongly at `text-[12px]`, corrected to Figma's `text-[16px]`/`text-[17px]`). Modify now switches the row into a real **edit-details** mode — every non-verified field becomes a controlled `<input>` in a bordered box, single "Update" button, values held in local `fieldOverrides` state and merged back over `stage.fields` on render (entries mode swaps the same Modify/Confirm ↔ Update buttons and adds a visual-only "+ Add Experience" row — its click just logs, since a real second-entry flow needs its own Q&A this session doesn't have). Completion label shows amber "In Progress" (not a stale percentage) for both `'in-progress'` and `'awaiting-review'` status. Props gained `confirming` (stage id mid-save → spinner on Confirm, both buttons disabled).                                                                                                                                                                                                                                                                      |
| `CareerBuddyHistoryDrawer` | `src/components/sections/profileFilling/CareerBuddyHistoryDrawer.jsx` | `5132:45345`                                                                                                                                     | Slide-in chat-history panel, grouped by date, verbatim entries from Figma.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `careerBuddyScript.js`     | `src/components/sections/profileFilling/careerBuddyScript.js`         | `5132:43308-46274`, `5132:40348`, `5132:52485`/`64380` (guidance)                                                                                | Deterministic conversation FSM (no `careerBuddy.service.js` yet) + panel field data. Guidance flow (2026-08-08): `guidance-intro*` → Career Exposure Q&A → Career Options confirm; other 6 guidance areas stubbed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `parentBuddyScript.js`     | `src/components/sections/profileFilling/parentBuddyScript.js`         | `5132:77628` / Guide `5132:78578`–`79168` / resume `5132:79750`                                                                                  | Parent FSM: welcome, ward setup, Guide ward edu+interests, **Build own** → talent `CAREER_BUDDY_NODES`, resume fill. `seedParentJump` + `PARENT_STAGE_SAVE_META`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `VoiceCallOverlay`         | `src/components/sections/profileFilling/VoiceCallOverlay.jsx`         | `5146:75913` / `75964`                                                                                                                           | Dual-avatar live voice call. Mic mute/unmute (real `getUserMedia`); X hang-up; settings → Choose a voice. Debug: `VoiceCallOverlay`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `VoiceSettingsOverlay`     | `src/components/sections/profileFilling/VoiceSettingsOverlay.jsx`     | `5146:76230` / `76269`                                                                                                                           | Choose a voice (Sol / Bruce / Tali). Done persists; Cancel discards. Data: `careerBuddyVoices.js`. Debug: `VoiceSettingsOverlay`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `AudioLevelWaveform`       | `src/components/ui/AudioLevelWaveform.jsx`                            | `5146:76424` / `76547`                                                                                                                           | 8-bar live waveform for voice-to-text; flat when silent. Driven by `useVoiceCapture` level.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `useVoiceCapture`          | `src/hooks/useVoiceCapture.js`                                        | —                                                                                                                                                | Real mic + AnalyserNode levels + Web Speech API transcript. Used by VTT input and voice call mute.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `sections/profileFilling/`          | 60+   | Career Buddy (chat + games + voice, see table above) + page-based stages: Interests/Skills intro+stage2, Work intro+stage2 (Add/Edit/Delete/Success modals, 2026-09-06), Portfolio intro+stage2 (Add/Edit/Delete/Success modals + project-type style map, 2026-09-07), Certs intro+stage2 (Add/Edit/Delete/Success modals + `certTypeStyles.js`, 2026-09-08/09), Goals intro+stage2 (Add/Edit/Delete/Success modals + `goalTypeStyles.js`, 2026-09-09), Pitch intro+stage2 (`PitchRecorder.jsx` + Published/Delete modals + `pitchModeStyles.js`, 2026-09-09) |

**Profile-filling stage flows — shared shape `✅ VERIFIED` (2026-09-09):** every page-based stage from Work onward ships the same six-file set — `<Stage>IntroSection.jsx` + `<Stage>IntroPage.jsx`, `<Stage>Stage2Section.jsx` + `<Stage>Stage2Page.jsx`, `AddEdit<Stage>Modal.jsx` (one component for both modes via a `mode` prop), `<Stage>SavedModal.jsx`, `Delete<Stage>Modal.jsx`, plus a `<stage>TypeStyles.js` data/helper module (needed because `react-refresh/only-export-components` forbids non-component exports from a component file). The intro page always reuses the shared `ProfileFillingJourneyPanel`; the stage-2 page always builds its own small right aside **inline** (there is deliberately no shared institution-style right-panel component for this flow). Routes are `/profile/filling/<stage>` and `/profile/filling/<stage>/<list-noun>`, registered in `App.jsx` and pointed at from `EngagementTopBar.jsx`'s `STAGE_ROUTES` map; each stage-2 "Next" CTA falls back to `/profile/engagement` until its successor stage exists, then is repointed. As of 2026-09-09 the built chain runs Interests → Skills → Work → Portfolio → Certs → Goals → **Pitch** (the 9th and final stage); only Personality still falls back to the engagement hub.

**Pitch breaks the shared shape on purpose `✅ VERIFIED` (2026-09-09):** the Pitch stage is the one page-based stage that is **not** a CRUD list, so it has **no `AddEdit…Modal.jsx`** and no card list. Figma ships exactly one pitch per profile expressed three mutually-exclusive ways, so `PitchStage2Section.jsx` is a single-item **mode switcher** (`record` / `upload` / `written`, plus a `null` empty state) at `/profile/filling/pitch/record`. Its file set is `PitchIntroSection.jsx` + `PitchIntroPage.jsx`, `PitchStage2Section.jsx` + `PitchStage2Page.jsx`, `PitchRecorder.jsx`, `PitchPublishedModal.jsx` (the success modal — named for Figma's own "Pitch published." headline rather than the sibling `…SavedModal` convention, because publishing is what it confirms), `DeletePitchModal.jsx` and `pitchModeStyles.js`. Being the last stage, its success CTA ("View your live profile") and every skip link route to `/profile/engagement` rather than to another filling stage.

### PitchRecorder details `✅ VERIFIED` (2026-09-09)

`src/components/sections/profileFilling/PitchRecorder.jsx` — the dark "Camera Viewfinder" (Figma `6083:46917` / `6107:57918` / `6083:47113`). **Not a `Modal` instance**: it is an inline block inside the stage-2 main column, with no scrim, no close button and no focus trap; the page's nav, top bar, aside and footer stay live around it.

| Prop                | Type                                                     | Notes                                                                   |
| ------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------- |
| `state`             | `'needs-access' \| 'ready' \| 'recording' \| 'recorded'` | Required. Drives which overlay/controls render                          |
| `elapsedSeconds`    | number (default `0`)                                     | Feeds the `mm:ss` timer; the owner runs the clock                       |
| `onAllowCamera`     | fn                                                       | "Allow camera" pill (mock — no `getUserMedia` call)                     |
| `onToggleRecording` | fn                                                       | 72px shutter; start ⇄ stop                                              |
| `onOpenSettings`    | fn                                                       | 44px settings button (Figma draws no settings panel — no-op by default) |
| `onPreview`         | fn                                                       | The recorded state's 72px play button                                   |

Figma draws only two of the four states (`needs-access` and `recorded`). `ready` is that same frame minus the access card and the "Allow camera" pill; `recording` is the same frame with the already-present REC dot turned `#ef4444` + pulsing, the already-present timer counting up, and the already-present shutter's inner circle morphing into a red rounded square. Both are pure subtractions/deltas from verified nodes — no invented chrome, no waveform (Figma draws none). **No real media capture anywhere:** nothing calls `navigator.mediaDevices.getUserMedia`, instantiates `MediaRecorder`, or persists a byte; the owner's `setInterval` clock and the design's real 60-second hard limit are the whole mechanism.

**Notable component note (2026-09-09):** `AddEditGoalModal.jsx` is the first stage modal with **four** numbered sections rather than three, and the first where Figma marks more fields required than the surrounding design supports — its salary inputs render Figma's green required asterisk but deliberately do not gate submission (the intro page's own stage card calls compensation range "Optional but high-value"). `GoalSalaryIcon` was added to `shared/assets.jsx` using path data copied verbatim from Figma's Dev-Mode asset server, since no money/currency glyph existed; every other icon in the Goals flow reuses an existing export. See `wiki/figma-node-map.md` § "Goals (Desired Career Options) — profile filling page flow" for the open `❓ NEEDS-CLARIFICATION` items carried into code comments.

**Notable component note (2026-09-09, Pitch):** the Pitch flow added **9 new icon exports** to `shared/assets.jsx` (`PitchFilmSlateIcon`, `PitchUploadTabIcon`, `PitchWriteIcon`, `PitchCameraVideoIcon`, `PitchComputerUploadIcon`, `PitchCameraIcon`, `PitchSettingsIcon`, `PitchBatteryIcon`, `PitchPlayIcon`, `PitchMedalIcon`, `PitchDropUploadIcon`) — all copied verbatim from Figma's Dev-Mode asset server with the baked-in colour swapped for `currentColor`. The pre-existing `PitchStepIcon` (the journey-panel/showcase glyph) was **not** duplicated, and `UploadCertificateIcon` / `WorkRoleIcon` / `CheckIcon` / `WorkEditPencilIcon` / `WorkDeleteWarningIcon` are reused as-is; `WorkDeleteWarningIcon` covers every delete affordance in the flow per the standing icon-consolidation instruction. The upload drop zone reuses the shared `ui/form/Upload.jsx` primitive (Figma's node is literally an instance of that design-system component) rather than re-drawing it inline, accepting two small visual deltas documented at the call site. The written-pitch textarea reuses `ui/form/Textarea.jsx` with `showCounter={false}` because Figma splits the counter into a left-hand word count and a right-hand DM-Mono character budget.

**Notable component note (2026-09-07):** `AddEditPortfolioModal.jsx`'s pinned-project toggle is a hand-rolled track/knob switch (`role="switch"`, `aria-checked`) — there is still no shared `Toggle`/`Switch` primitive under `ui/form/` (checked before building; only `Checkbox` exists, and its shelf-button visual language doesn't fit a toggle). Track a shared `Toggle` component if a 3rd caller needs this same visual, per this codebase's "3rd duplicate → extract to shared" rule (see `ProfileFillingJourneyPanel.jsx`'s own header comment for the precedent). Separately, this build's browser verification pass caught 4 pre-existing kebab-case SVG prop bugs in `shared/assets.jsx` (`BriefcaseIcon`, `WorkEditPencilIcon`, `WorkDeleteWarningIcon`) firing React "Invalid DOM property" console warnings — fixed in place (see `wiki/log.md` 2026-09-07 entry).
