...
**Figma MCP note:** `mcp__figma__get_design_context` worked for every node in this dive. `get_metadata` was used **only** to discover which child frames to dive into — nothing in this flow is styled or worded from a layer name. That mattered here: the success screen's underlying page (`6107:122287`–`122461`) still carries stale Interests-era layer names ("What pulls you in?", "🌱 Why interests matter", "📊 Popular in Ghana") that a metadata-only read would have implemented as real content.

**This stage is deliberately NOT a CRUD list.** Certs/Work/Portfolio/Goals are all "add N entries" flows. Figma ships exactly ONE pitch per profile, expressed three mutually-exclusive ways — record in-browser, upload a file, or write four sentences. There is no Add/Edit modal, no card list and no "Add another" affordance anywhere in the Pitch node range, so `PitchStage2Section.jsx` is a single-item **mode switcher** and the route is `/profile/filling/pitch/record` rather than `/list`. Consequently there is no `AddEditPitchModal.jsx`, and the success modal is named `PitchPublishedModal.jsx` (after Figma's own "Pitch published." headline) rather than following the `…SavedModal` convention.

**New files:**

- `src/components/sections/profileFilling/PitchIntroSection.jsx` + `src/pages/profileFilling/PitchIntroPage.jsx` — mirrors `GoalsIntroSection.jsx` (header, "What this stage covers" + 4 numbered info cards, shared `ProfileFillingJourneyPanel` with `activeStageId="talent-pitch"`). All four intro cards are genuinely Pitch-specific — no cross-stage clone leftovers on this page, unlike the Goals intro's "Cover images" card.
- `src/components/sections/profileFilling/PitchStage2Section.jsx` + `src/pages/profileFilling/PitchStage2Page.jsx` — the mode switcher. `mode === null` renders Figma's empty state (dashed panel, "No pitch yet.", "Add your pitch" / "Write a pitch", "Three ways to pitch" cards, the compact 4-sentence-formula card); `record` renders the viewfinder, and once a take exists swaps to the playback scrubber + "Quick self-check" + green Re-Record action bar + 4 checkbox rows + the 100% profile-strength card + footnote; `upload` renders the shared `Upload` drop zone and the phone-transfer cards, or once a file is picked the preview viewfinder + file row + "File passed all checks" grid + "Before you publish : quick check" list; `written` renders the required textarea + word/char counters + the numbered 4-sentence tips card + the "Want 6× more recruiter messages?" / "Record Instead" banner. Plus an inline `PitchStage2RightAside` whose three cards' copy switches per mode/state.
- `src/components/sections/profileFilling/PitchRecorder.jsx` — the dark "Camera Viewfinder". **Not** a `Modal` instance: verified inline block, no scrim/close/focus-trap, page chrome stays live around it. Full prop table in `wiki/components.md` § "PitchRecorder details".
- `src/components/sections/profileFilling/PitchPublishedModal.jsx` — success modal (🏆, mixed-style "Pitch **published.**", "Your profile is 100% complete" checklist row, 3-row mini-list with ✓ Published / Anytime / ✓ Passed pills, "100% : 9 stages done" bar, gold "View your live profile" CTA → `/profile/engagement`).
- `src/components/sections/profileFilling/DeletePitchModal.jsx` — delete-confirm with a pink pitch preview card (● Live pill, amber format line, blue "Preview ↗" chip + neutral chips), two reason rows, and a "Replace Instead" nudge.
- `src/components/sections/profileFilling/pitchModeStyles.js` — mode/tab constants, per-state aside copy, the three-ways cards, both presentations of the 4-sentence formula, phone-transfer cards, both pre-publish checklists, the seeded demo file, and the `formatRecorderTime` / `formatDurationLabel` / `countWords` / `buildPitchSummary` / `buildDeletePreview` / `buildUploadFileChecks` / `buildUploadAsideChecks` helpers. Split out per the `react-refresh/only-export-components` rule (same reason `certTypeStyles.js` / `goalTypeStyles.js` exist).

**Modified files:**

- `src/App.jsx` — registered `/profile/filling/pitch` and `/profile/filling/pitch/record`.
- `src/components/sections/engagement/EngagementTopBar.jsx` — `STAGE_ROUTES['talent-pitch']` changed from the `/profile/engagement` placeholder to the real `/profile/filling/pitch` intro route. **Every one of the 9 stages except Personality now has a real route.**
- `src/components/sections/profileFilling/GoalsStage2Section.jsx` — `handleNext` now routes to `/profile/filling/pitch`; the "Pitch not yet built" fallback comment removed.
- `src/components/shared/assets.jsx` — the Pitch glyph block (started by an earlier interrupted attempt) was verified complete: `PitchFilmSlateIcon`, `PitchUploadTabIcon`, `PitchWriteIcon`, `PitchCameraVideoIcon`, `PitchComputerUploadIcon`, `PitchCameraIcon`, `PitchSettingsIcon`, `PitchBatteryIcon`, `PitchPlayIcon`, `PitchMedalIcon`, `PitchDropUploadIcon` — all Figma-verbatim path data with `currentColor` substituted, nothing hand-drawn. The pre-existing `PitchStepIcon` was **not** duplicated; `UploadCertificateIcon` / `WorkRoleIcon` / `CheckIcon` / `WorkEditPencilIcon` / `WorkDeleteWarningIcon` are reused, with `WorkDeleteWarningIcon` covering every delete affordance per the standing instruction.
- `src/constants/profileStages.js` — **checked, unchanged.** Its `talent-pitch` entry is already correct (`trailLabel: 'Pitch'`, stage 9). Its `durationLabel` fields were deliberately left alone per instruction — nothing renders them in the two locations the sweep targeted.

**Shared components reused rather than re-drawn:** `ui/form/Upload.jsx` for the drop zone (Figma's `6083:46977` is literally an instance of the design system's own Upload component, symbol `3014:50687`), `ui/form/Textarea.jsx` for the written pitch (`showCounter={false}` because Figma splits the counter in two), `ui/form/Checkbox.jsx` for all eight checklist rows, `ui/Modal.jsx` + `ui/Button.jsx` for both overlays. Two knowingly-accepted deltas on the drop zone, documented at the call site: the shared primitive paints a 2px dashed `#c1d4c4` border on `#fefefd` where the Figma instance overrides to 1.2px dashed `#387440` on `rgba(235,241,236,0.5)` (not overridable from the call site without restyling the primitive for every other page), and its link reads "browse to choose a file" where Figma's own string has the typo "**browser** to choose a file".

**Verbatim-fidelity calls (all reproduced as designed, all flagged inline):** the empty state's Certs-leftover "certificate-01" glyph (`6083:34047`); the success modal's subtext, which literally reads "…now see and hear you not just read about you." with two spaces where an em dash clearly belongs (`6107:122571`); the record checklist's doubled space in "The audio is clear I can hear myself without straining" (`5906:17`); the upload checklist's space before its comma in "The audio is clear , I can hear myself…" (`5903:53`); the same four file facts written with a **colon** separator in the on-page grid but an **em dash** in the aside; the stage-2 footer's literal "Goals" back-button label; the intro right panel's Portfolio-clone impact label ("more recruiter focus on portfolio", `5625:93093`) whose trailing noun alone was swapped, grounded in this page's own verified subtext.

**Judgment calls (documented inline, not blind copies):**

- **The recording window's interaction model.** Figma ships only two viewfinder states — "camera access needed" (`6083:46917`) and "recorded / click to preview" (`6107:57918`) — yet the flow must pass through a permission-granted idle state and an actively-recording one (the intro's own card #2 says "Record as many takes as you need"). Both missing states are rendered as **minimal deltas from the verified frames**: `ready` is that frame minus the access card and the "Allow camera" pill; `recording` turns the already-present REC dot `#ef4444` + pulsing, counts the already-present timer up, and morphs the already-present 72px shutter's inner circle into a red rounded square. No new chrome, no invented copy, and **no waveform** (Figma draws none anywhere in this flow). The 60-second auto-stop is real — the design's own hard limit, stated twice in verified copy ("Keep it under 60s", "Maximum 60 seconds").
- **No real media capture.** Nothing calls `navigator.mediaDevices.getUserMedia`, instantiates `MediaRecorder`, or persists a byte — same "no invented backend" rule as every file-upload button elsewhere. "Allow camera" flips a UI flag; the shutter starts/stops a `setInterval` clock. A picked upload file is read for its real name/size (the browser gives those) but duration would need decoding, so Figma's own "0:54" stands in via `DEMO_PITCH_FILE`.
- **Mode tab row rendered in Record mode too.** Figma draws the Record/Upload/Written pill row only on the Upload frames (`6120:134569`, `6107:134149`) and the Written frame (`6120:134598`) — not on either Record frame or the empty state. Without it there'd be no way back to Upload or Written once recording starts, so it renders in all three modes (never on the empty state, matching Figma).
- **The Written frame's "Upload" pill holds two icons** — `edit-01` (`6120:134614`) _and_ `upload-05` (`6120:134627`) — an authoring slip; only the upload glyph is rendered.
- **Delete entry point.** No Pitch frame draws one, yet Figma ships a complete Delete-Pitch modal. A 28px delete button (identical styling to the sibling stages' card affordance, using `WorkDeleteWarningIcon`) sits beside the "Your Pitch" section label whenever a pitch exists. Flagged for design confirmation.
- **Footer CTA label.** Two footers overlap on every Pitch frame. The recorded screen's own older footer (`5890:2068`) says "Publish & go live"; the newer shared footer component pasted over EVERY frame in the range (`6686:914xx`/`915xx`) — painted last, so it is what actually shows — says "Review & publish". The shared, on-top, universally-present label is used. The button is disabled until a pitch actually exists (Figma draws no disabled state, but publishing nothing isn't a real action — documented business-logic call).
- **Success/delete summaries are templates, not literals.** Figma hard-codes a video pitch ("60-second video pitch" / "MP4 · 0:54 · 18.4 MB · uploaded securely"). Since this flow can also publish a written pitch, `buildPitchSummary` / `buildDeletePreview` preserve Figma's sentence shapes and substitute live state — same convention `CertSavedModal` / `GoalSavedModal` already use.
- **The upload file-checks grid is derived, not static.** Caught during browser verification: Figma's hard-coded "18.4 MB" in the 2×2 grid contradicted the live "18.0 MB" in the file row directly above it. `buildUploadFileChecks` / `buildUploadAsideChecks` now fill Figma's own sentence shapes with the picked file's real format/size (duration still Figma's, since it can't be read).
- **Success-modal CTA left icon dropped.** Figma's CTA has two icon slots; the left one it names "loader" resolves to the _same asset hash_ as the delete modal's trash glyph — the button component's unset default forwardicon leaking through, not a deliberate icon. Only the right-hand arrow is rendered. Flagged.
- **Required-field logic.** Unlike every prior flow, Figma's only required marker here is a genuine one: the written-pitch textarea's green `*` (`6120:134680`/`134681`, `#2e8b57`). No stray decorative `#fb7185` asterisks at off-frame offsets exist in the Pitch node range (checked). Publishing is gated on "a pitch of the active mode exists", not on the checklists — those are self-check prompts, not validation, per their own copy.
- **Mixed-style headlines checked per-headline, not assumed.** The intro headline (`5625:93160`) IS mixed (plain "Pitch" + italic green "Your 60-second story."); the stage-2 headline (`5978:29455`) is NOT (one plain `#111` node across both lines); the success headline (`6107:122570`) IS ("Pitch " + italic green "published."). All three verified in the raw dive.

**🔧 Cross-flow duration-consistency sweep (explicit user instruction).** Rule adopted: **each flow's own intro-page tag row is the single source of truth** for that flow's approximate duration, and it is reconciled into (1) that flow's intro right-panel "Time to complete" card and (2) that flow's stage-2 header tag row. `src/constants/profileStages.js`'s `durationLabel` fields were deliberately NOT touched (nothing renders them in either target location).

| Flow      | Intro tag (source of truth) | Right panel                       | Stage-2 tag                    |
| --------- | --------------------------- | --------------------------------- | ------------------------------ |
| Work      | `~5 min` (unchanged)        | `~8 min` → **`~5 min`**           | `~5 min` (already correct)     |
| Portfolio | `~7 min` (unchanged)        | `~8 min` → **`~7 min`**           | `~6 min` → **`~7 min`**        |
| Certs     | `~7 min` (unchanged)        | `~7 min` (already correct)        | `~7 min` (already correct)     |
| Goals     | `~7 min` (unchanged)        | `~4 min` → **`~7 min`**           | `~3 min` → **`~7 min`**        |
| Pitch     | `~7 min` (unchanged)        | `~7 min` (set correctly at build) | Figma's `2 min` → **`~7 min`** |

Files touched by the sweep: `WorkIntroSection.jsx`, `PortfolioIntroSection.jsx`, `PortfolioStage2Section.jsx`, `GoalsIntroSection.jsx` (plus its now-resolved clarification #2 note), `GoalsStage2Section.jsx`, `CertsIntroSection.jsx` (comment-only — it claimed the panel rendered Figma's "~4 min" when it has always rendered "~7 min"), and `PitchStage2Section.jsx`. Every change carries an inline `🔧 DURATION-CONSISTENCY SWEEP` comment naming the old value, the Figma node it came from, and the fact that the divergence is instructed. Browser-verified per flow: Goals intro 2×"~7 min" / list "~7 min"; Portfolio intro 2×"~7 min" / projects "~7 min"; Work intro 2×"~5 min" / history "~5 min"; Certs intro 2×"~7 min" / list "~7 min"; Pitch intro 2×"~7 min" / record "~7 min".

**Verified in browser** (`mcp__Claude_Browser__*`, 1440×950, dev server started from `.claude/launch.json`): Pitch intro renders in full (mixed-style headline, subtext, 3 tags, 4 numbered cards, footer, right panel showing Pitch / 60% / ~7 min / journey with Pitch active, top bar "Step 9 of 9 · Pitch · 90% profile complete"). Stage-2 empty state → clicked "Add your pitch" → record mode: `needs-access` (grid lines, focus brackets, 4K/24fps, battery, REC pill, glass access card, Allow camera + shutter + settings, "00:00"), `ready` after allowing (card and pill gone, shutter live), `recording` after the shutter (red pulsing dot, counting timer, red stop square), then the real 60-second auto-stop fired on its own and produced `recorded` (preview card + "Click to preview", scrubber "0:00 … 1:00", "Quick self-check", green Re-Record bar, 4 checkbox rows, 100% profile-strength card + medal, footnote; aside swapped to "Quick self-check" / "Perfection is the enemy"). Ticked a checkbox, published → success modal with the live recorded summary ("WebM · 1:00 · recorded in-browser · uploaded securely" + ✓ Published) and the aside's recruiter-views jumping 0 → 24. Delete modal verified from the recorded state (preview card, ● Live, amber format line, Preview ↗ + chips, both reason rows, Replace Instead, Cancel + destructive CTA); "Permanently delete this pitch" correctly reset the page to the empty state with the aside and publish button back to their zero states. Upload mode verified in both states (drop zone with Mp4/Mov/WebM pills and phone-transfer cards; then file row, derived checks grid, pre-publish checklist, bottom links). Written mode verified (required label with green `*`, placeholder, live "24 words" / "136 / 600", numbered tips card, COMPLETE EXAMPLE + meta line, upsell banner with "Record Instead", skip link) and published → success modal showing "Written pitch" / "24 words · ~6 seconds to read · saved securely". "View your live profile" navigated to `/profile/engagement`. Goals stage-2 "Next: Pitch" → Pitch intro confirmed via the `[PitchIntroSection] mount` log. The top-bar trail's "Pitch" item now resolves to `href="/profile/filling/pitch"` (checked from the Certs list page).

**Console:** re-checked on a **freshly opened tab** (this Browser pane's console buffer is a persistent ring that survives reloads, so a same-tab reload keeps replaying pre-fix history) — zero errors, only the pre-existing benign React Router v7 future-flag warnings. The `debug()` logging convention is live throughout (`[PitchIntroSection]`, `[PitchStage2Section]`, `[PitchRecorder]`, `[PitchPublishedModal]`, `[DeletePitchModal]`) covering mount, mode/state branches, the recording clock start/stop, the 60s limit, file picks, checklist toggles, publish and delete.

**Verification:** `npm run lint` clean (only the pre-existing unrelated `OnboardingRightPanel.jsx` `react-hooks/exhaustive-deps` warning). `npm run build` clean (only the pre-existing repo-wide >600 kB chunk-size advisory; bundle 2,114.07 kB / CSS 166.12 kB).

**One layout fix made during verification:** the mode tab row was initially rendered above the "Your Pitch" section label; Figma puts the label first on all three frames that carry the row (`5890:694` / `5890:1040` / `5890:1386`), so the label was hoisted out of the per-mode renderers to sit above the tabs at the container level.

**Known gaps flagged, not fixed:**

- Switching modes does not discard the other modes' in-progress content (an uploaded file survives a switch to Written, etc.). Only the active mode's content is publishable, which is the sane read, but Figma specifies no cross-mode discard behaviour.
- The Certifications build (2026-09-08/09) still has no `wiki/log.md` entry of its own, even though its own source files reference a `figma-node-map.md` § "Certifications — profile filling page flow" that does not exist. Carried forward from the Goals entry below; still not back-filled.
- `PROFILE_STAGES` still marks every stage except Avatar `NOT_STARTED`, so the top bar's completion percentages are per-page literals (`completionPct={90}` here, Figma-verified) rather than derived. Unchanged, pre-existing.

## [2026-09-09] fix | Kebab-case SVG props across `shared/assets.jsx` and `AddSkillModal.jsx` (React "Invalid DOM property" console errors)

Closes the gap the Goals build flagged in its own log entry below ("Widespread pre-existing kebab-case SVG props remain... spawned as a separate task").

Many inline SVG icon components used raw HTML/SVG attribute names (`stroke-width`, `stroke-linecap`, `stroke-linejoin`, `stroke-opacity`, `fill-rule`, `clip-rule`, `stop-color`) instead of React's camelCase JSX props. React DOM rejects these at runtime — logging `Invalid DOM property 'stroke-width'. Did you mean 'strokeWidth'?` (etc.) and **silently dropping the attribute**, so stroke widths, line caps, gradient stop colours, and fill/clip rules never actually applied. These errors fired on every profile-filling page (the affected icons render via `EngagementTopBar` and `ProfileFillingJourneyPanel`), violating this repo's "browser console errors are real bugs" rule.

**Verified scope** via `grep -rn` across `src/components` and `src/pages` for every kebab-case SVG presentation attribute (`stroke-*`, `fill-*`, `clip-*`, `stop-*`, plus the fuller marker/text/paint-order family, none of which were present) — confirmed exactly two source files affected; every other hit was inside a real standalone `.svg` file under `src/assets/` (correctly kebab-case there, not touched — those are loaded via `<img src>`/URL, never inlined as JSX):

- `src/components/shared/assets.jsx` — `AvatarStepIcon`, `InterestsStepIcon`, `PersonalityStepIcon`, `SkillsStepIcon`, `WorkStepIcon`, `PortfolioStepIcon` (the "Your journey" step-list icons in `ProfileFillingJourneyPanel`), plus several more icon components further down the file. Converted every occurrence via project-wide `stroke-width`→`strokeWidth`, `stroke-linecap`→`strokeLinecap`, `stroke-linejoin`→`strokeLinejoin`, `stroke-opacity`→`strokeOpacity`, `fill-rule`→`fillRule`, `clip-rule`→`clipRule`, `stop-color`→`stopColor`.
- `src/components/sections/profileFilling/AddSkillModal.jsx` — `GameControllerIcon` (~lines 46-90): `stroke-width`→`strokeWidth`, `stroke-linecap`→`strokeLinecap`, `stroke-linejoin`→`strokeLinejoin`.

(`UploadCertificateIcon` in `assets.jsx` was already fixed in the Goals build session immediately prior to this one — used as the reference conversion pattern here.)

**Verification:** `npm run build` clean (only the pre-existing repo-wide >600 kB chunk-size advisory), `npm run lint` clean (only the pre-existing unrelated `OnboardingRightPanel.jsx` `react-hooks/exhaustive-deps` warning). Browser-verified on a **freshly opened tab** (important: this Browser pane's console-message buffer is a persistent ring that survives page reloads/navigations, so a reload alone kept surfacing the old pre-fix errors from earlier in the same tab's history — closing and reopening the tab was needed to get a clean read) at `/profile/filling/certs` and `/profile/filling/goals/list`: zero "Invalid DOM property" errors, only the pre-existing benign React Router future-flag warnings. Screenshotted the "Your journey" step-icon list (Avatar/Interests/Personality/Skills/Work/Portfolio/Certs/Goals/Pitch) at 1440×1000 — all icons render with correct stroke weights and rounded caps, no visual regressions from the previously-dropped attributes now actually applying.

## [2026-09-09] create | Goals (Desired Career Options) profile-filling page flow (intro + list + Add/Edit/Delete/Success modals)

Built `/profile/filling/goals` (+ `/profile/filling/goals/list`) — the **8th of 9** profile-filling stages (Avatar → Interests → Personality → Skills → Work → Portfolio → Certs → **Goals** → Pitch), `PROFILE_STAGES` id `desired-career`, trail label "Goals". Same file key `Bin8roWL8sloyc36IgFMuT`, node range `5619`/`5622`/`5625`/`5659`/`6286`/`6668`/`6686` — full node inventory in `wiki/figma-node-map.md` § "Goals (Desired Career Options) — profile filling page flow".

**Figma MCP note (the headline difference from the Certs build):** `mcp__figma__get_design_context` worked for **every** node in this dive. Nothing in this flow is derived from `get_metadata` layer names — `get_metadata` was used only to discover which child frames to dive into. Where Figma's own copy is a cross-stage clone leftover it is reproduced verbatim and flagged inline, never silently replaced with invented copy.

**New files:**

- `src/components/sections/profileFilling/GoalsIntroSection.jsx` + `src/pages/profileFilling/GoalsIntroPage.jsx` — mirrors `CertsIntroSection.jsx` (header, "What this stage covers" + 4 numbered info cards, shared `ProfileFillingJourneyPanel` with `activeStageId="desired-career"`).
- `src/components/sections/profileFilling/GoalsStage2Section.jsx` + `src/pages/profileFilling/GoalsStage2Page.jsx` — the list page. Empty state ("No goals yet." + "Add your first goal" / "Add Manually" + an "Example of a complete goal" reference card), populated state (`GoalCard`: 44px laptop tile, `role — industry` title, money glyph + salary line, blue opportunity-type chip + grey location/timeline chips, green quoted description), an inline `GoalsStage2RightAside` ("What counts?" / "Why this matters" / "👀 Recruiter views this week") and a full-width footer (Goals ← / → Next: Pitch).
- `src/components/sections/profileFilling/AddEditGoalModal.jsx` — single form for both Add and Edit (`mode` prop), **four** numbered sections (Role & opportunity type / Location & timeline / Salary / compensation range / Notes for recruiters).
- `src/components/sections/profileFilling/GoalSavedModal.jsx` — success modal (🏆, "Goals complete.", "Goal-matched search active" checklist, mini goal-list preview, static "78% : 8 stages done" bar, gold "Continue to Pitch stage" CTA). Opens after every successful save (add AND edit), same rule every sibling stage established.
- `src/components/sections/profileFilling/DeleteGoalModal.jsx` — delete-confirm with a pink goal preview card (Active pill, primary-goal line, ★ Primary + neutral chips), two reason rows, and an "Edit instead" nudge.
- `src/components/sections/profileFilling/goalTypeStyles.js` — industry / opportunity-type / timeframe option lists + shared chip-style constants + `buildSalarySummary` / `buildGoalSummaryLine`, split out per the `react-refresh/only-export-components` rule (same reason `certTypeStyles.js` exists).

**Modified files:**

- `src/App.jsx` — registered `/profile/filling/goals` and `/profile/filling/goals/list`.
- `src/components/sections/engagement/EngagementTopBar.jsx` — `STAGE_ROUTES['desired-career']` changed from the `/profile/engagement` placeholder to the real `/profile/filling/goals` intro route.
- `src/components/sections/profileFilling/CertsStage2Section.jsx` — `handleNext` now routes to `/profile/filling/goals` instead of the documented "Goals not yet built" fallback; comment updated.
- `src/components/shared/assets.jsx` — added `GoalSalaryIcon` (the 12px "money-03" glyph on every goal card; path data copied verbatim from the Figma Dev-Mode asset server's own SVG, `#387440` → `currentColor` — no hand-drawn approximation, because assets.jsx had no money/currency glyph at all). Also fixed 3 pre-existing kebab-case SVG props on `UploadCertificateIcon` (`stroke-width`/`stroke-linecap`/`stroke-linejoin`) that were firing React "Invalid DOM property" console errors; every other Goals icon **reuses** an existing export (`WorkRoleIcon` for the laptop tile, `UploadCertificateIcon` for the empty-state glyph, `WorkDeleteWarningIcon` for every delete affordance per explicit instruction, `WorkEditPencilIcon` for edit).

**Canonical vs stale Figma artboards (important):** three populated Goals list artboards exist. `5619:81443` is canonical (correct onboarding chrome, four data points per card). `5622:89666` and the list inside `5619:82728` are older duplicates wearing the marketing-site nav, an Interests-stage "What pulls you in?" header, "📊 Popular in Ghana" asides, and Work-stage card fields ("Jan 2024 – Present", "1yr 7mo", "● Active", "↕ Re-order"). Only the canonical shape was implemented.

**Verbatim-fidelity calls (all reproduced as designed, all flagged inline):** intro card #4's title "Cover images" against an NGO/impact body; the intro page's own "~7 min" tag vs its right panel's "~4 min"; three cross-stage placeholder leftovers in the modal ("e.g. Google Data Analytics Certificate…", "e.g. Solo developer — Lead UX designer…", "https://…"); the empty state's certificate glyph and its "No certs yet?? Skip this stage" line; the delete modal's Certs-flavoured "Wrong date, wrong org name, missing credential ID?" nudge; the stage-2 footer's literal "Goals" back-button label. The Edit modal's badge and headline are byte-identical to Add mode in Figma itself (re-dived to confirm) — reproduced, since there is no second source to substitute from.

**Judgment calls (documented inline, not blind copies):**

- **Salary required-markers.** Figma puts genuine green required asterisks on six fields including both salary inputs, but the intro page's own stage card says compensation range is "Optional but high-value" and the modal's own salary note is a persuasive pitch, not a requirement notice. All six asterisks render exactly as designed; `canSubmit` gates only on role / skills / location / description. Same either-or resolution `AddEditCertModal.jsx` reached for its Credential-ID / Badge-URL pair.
- **Stray pink asterisks.** Every modal section frame carries two `#fb7185` "\*" nodes at off-frame absolute offsets (e.g. `left: -495.83px`) — file artifacts, not required-markers, not reproduced. Same class of artifact the Portfolio build flagged.
- **Success-modal badges.** Figma's mini-list shows "✓ Verified" / "Self-reported" — verbatim Certs badges with no Goals-side concept behind them and no rule that could assign them. Replaced with the Goals-native "★ Primary" chip (a real, verified badge from the delete modal, whose meaning the rest of the flow supports) on the primary row only.
- **Section 2's second select** is Figma-labelled "Opportunity type" / "Any type" — duplicating Section 1's own label inside a "Location & timeline" section — while every card renders a timeline chip no field produces. Wired to the timeline chip with Figma-verbatim label and placeholder; its options are exactly the five timeline strings Figma's own cards contain.
- **Card edit/delete buttons** are carried over from the Certs/Work/Portfolio pattern; the canonical Goals frame shows no such affordance even though a full Edit modal and Delete modal ship.
- **Drag-to-reorder** (described in copy twice, shown as a "↕ Re-order" chip only on stale artboards) is NOT implemented; the first list entry is treated as the primary goal.

**Verified in browser** (dev server already running on 5173, `mcp__Claude_Browser__*`, 1440×900): intro page renders in full (headline, subtext, 3 tags, 4 numbered cards, footer, right panel showing Goals / 60% / ~4 min / journey with Goals active, top bar "Step 8 of 9 · Goals · 78% profile complete"). List page populated state renders all 5 seed goals with correct salary lines and chip rows. Add modal: all four sections render, both Industry and Opportunity-type dropdowns populate, the timeline select populates, `Save Goals` stays disabled until the four gated fields are filled, then saves and prepends the new goal. Success modal renders with live counts, checklist row, mini list with "★ Primary" on the top goal, and the profile-strength bar. Delete modal verified in BOTH states — non-primary goal (no ★/amber line) and primary goal (Active pill + amber "Primary goal · drives your headline match" + ★ Primary chip). "Edit Instead" correctly closes delete and opens the pre-filled Edit modal with its "Delete this Goal" + "Save Changes" footer. Emptied the list to verify the empty state (dashed panel, certificate glyph, both CTAs, "No certs yet?? Skip this stage", the "Example of a complete goal" card, and the aside's 0-view / "Set your goals to unlock recruiter discovery" state).

`npm run lint` clean (only the pre-existing unrelated `OnboardingRightPanel.jsx` `react-hooks/exhaustive-deps` warning). `npm run build` clean (only the pre-existing repo-wide >600 kB chunk-size advisory).

**Known gaps flagged, not fixed:**

- Widespread **pre-existing** kebab-case SVG props remain in `shared/assets.jsx` (`AvatarStepIcon`, `InterestsStepIcon`, `PersonalityStepIcon`, `SkillsStepIcon`, `WorkStepIcon`, `PortfolioStepIcon`, `PortfolioPostIcon`, `HelperTextIcon`, `ImagePlaceholderIcon`, …) and in `AddSkillModal.jsx`, still logging "Invalid DOM property" console errors on every profile-filling page. Out of scope here (touches many already-shipped pages); spawned as a separate task.
- The **Certifications** build (2026-09-08/09) never added its own `wiki/log.md`, `wiki/components.md` or `wiki/figma-node-map.md` entries, even though `CertsIntroSection.jsx` / `CertsStage2Section.jsx` reference a `wiki/figma-node-map.md` § "Certifications — profile filling page flow" that does not exist. Noted here so it isn't lost; not back-filled in this pass.

## [2026-09-07] create | Project Portfolio profile-filling page flow (intro + list + Add/Edit/Delete/Success modals)

Built `/profile/filling/portfolio` (+ `/profile/filling/portfolio/projects`) — the 6th of 9 profile-filling stages (Avatar → Interests → Personality → Skills → Work → **Portfolio** → Certs → Goals → Pitch), structurally mirroring the Work Experience flow built the previous day (same file key `Bin8roWL8sloyc36IgFMuT`, node range `5178`/`5180`/`5200`/`5204`/`5209`/`5211`/`5215`/`5217`/`6286` — full detail in `wiki/figma-node-map.md` § "Project Portfolio — profile filling page flow").

**New files:**

- `src/components/sections/profileFilling/PortfolioIntroSection.jsx` + `src/pages/profileFilling/PortfolioIntroPage.jsx` — mirrors `WorkIntroSection.jsx` (header, "what this stage covers" + 4 numbered info cards, shared `ProfileFillingJourneyPanel` right panel with `activeStageId="project-portfolio"`).
- `src/components/sections/profileFilling/PortfolioStage2Section.jsx` + `src/pages/profileFilling/PortfolioStage2Page.jsx` — the list page. Empty state ("No projects yet.") and populated state (`ProjectCard`: cover-image/gradient block, pinned badge, edit/delete icon buttons, link + tech tag row), a right aside ("What counts?" / "Popular in Ghana" / "Recruiter views this week"), and a footer (Work ← / → Certs).
- `src/components/sections/profileFilling/AddEditPortfolioModal.jsx` — single form for both Add and Edit (via `mode` prop), 4 sections (Cover image upload, Project basics, Description, Technologies & links) + a hand-rolled pinned-project toggle switch (no shared Toggle primitive exists yet in `ui/form/`).
- `src/components/sections/profileFilling/DeletePortfolioModal.jsx` — delete-confirm with a project preview card (type-colour strip + icon), two reason rows, and an "Edit instead?" nudge.
- `src/components/sections/profileFilling/PortfolioSavedModal.jsx` — success modal (trophy, live project/pinned-count summary, "Portfolio filter unlocked" checklist, mini project-list preview, static profile-strength bar); opens after every successful save (add AND edit), same rule `WorkSavedModal.jsx` established.
- `src/components/sections/profileFilling/portfolioProjectTypeStyles.js` — project-type → accent-colour gradient map (invented, flagged) + year-options helper, split out per the `react-refresh/only-export-components` rule (same reason `workRoleStatusStyles.js` exists).

**Modified files:**

- `src/App.jsx` — registered the two new routes alongside the other profile-filling routes.
- `src/components/sections/engagement/EngagementTopBar.jsx` — `STAGE_ROUTES['project-portfolio']` changed from the `/profile/engagement` placeholder to the real `/profile/filling/portfolio` intro route.
- `src/components/sections/profileFilling/WorkStage2Section.jsx` — `handleNext` now routes to `/profile/filling/portfolio` instead of falling back to `/profile/engagement`, since the Portfolio flow this comment referenced not existing now exists.
- `src/components/shared/assets.jsx` — fixed 4 pre-existing kebab-case SVG prop bugs (`stroke-width`/`stroke-linecap`/`stroke-linejoin` on `BriefcaseIcon` and `WorkEditPencilIcon`; `fill-rule`/`clip-rule` on `WorkDeleteWarningIcon`) that were firing "Invalid DOM property" console warnings on every page using those icons — found while browser-verifying this Portfolio flow, which uses `WorkDeleteWarningIcon` and `WorkEditPencilIcon` far more heavily than Work's own pages did.

**Verbatim-fidelity calls (this modal's Figma frames were evidently cloned from the Skills "Add a skill" modal and only partially re-copy-edited):** the small status badge above the Add/Edit-modal headline reads "Add A skill" in BOTH modes; the subtitle under it reads "Fill in the name, set your proficiency honestly, and tag it to a category. That's it — you can verify it later." in BOTH modes (Skills-flow concepts with no equivalent here); the "Figma / design file link" field's helper text is word-for-word identical to the "Your specific role" field's helper a few rows up. All three reproduced verbatim per this session's "never silently fix Figma copy" rule (flagged in code comments for design review). The delete-modal's description also reads "...remove the role from your work history..." — verbatim Work-flow copy, likewise kept verbatim and flagged. A hidden "Skills Lab — verify Excel & SQL next" node in the same modal tree (`hidden="true"` in Figma's own layer tree) further confirms the clone — respected by NOT rendering it, same treatment `AddEditWorkModal.jsx` gave its own hidden "Editing banner". Figma's own section-numbering badges show "2" on both the Cover-image AND Project-basics sections (an authoring slip) — renumbered sequentially 1-4 in code for UX clarity. The "Required Indicator" asterisk nodes on the Technologies & links section are positioned via absolute offsets landing off-frame (`left: -495.83px`) — not reliable evidence of which fields are required, so Live-link/GitHub/Figma-link were all made optional at the code level (business-logic call, documented inline) rather than literally requiring both Live-link and Figma-link as a naive asterisk count would suggest.

**"Only one can be pinned" invariant:** `PortfolioStage2Section.jsx`'s `handleSaveProject` un-pins every other project whenever a save sets `isPinned: true` — real business logic evidenced by the pin-toggle's own copy in both Add and Edit modes of `AddEditPortfolioModal.jsx`.

**`WorkDeleteWarningIcon` reused everywhere** a delete icon is needed in this flow (list-row delete button, delete-modal header badge, edit-modal's "Delete this Project" button) per explicit instruction, even where Figma's own dive shows a distinct plain trash-can "20-delete" glyph for some of those spots — a deliberate icon-set consolidation, documented inline.

**Verified in browser:** full click-through at `/profile/filling/portfolio` and `/profile/filling/portfolio/projects` — intro page renders (journey panel, info cards), list page populated state (3 seed projects with distinct type-colour gradients: green/Web app, blue/Mobile app, purple/Design case study, amber/Data analysis), Add project (all 4 sections + Select dropdowns for Project Type/Year + pin toggle all work, new project prepends to list), Edit project (pre-fills real values, e.g. "Accra Bus Tracker"), Delete project (confirm dialog shows correct project preview), Success modal (live project/pinned counts, confirmed the pin invariant — pinning a new project during Add correctly un-pinned "Accra Bus Tracker"). No console errors after the `assets.jsx` icon-prop fix. `npm run build` and `npm run lint` both clean (only the pre-existing unrelated `OnboardingRightPanel` lint warning). `npm run format:check` flags both new and many pre-existing files repo-wide (91 total) — this is pre-existing repo-wide Prettier drift, not new drift introduced here; not addressed (out of scope — would touch unrelated files).

**Not built / explicitly deferred:** the two small icon-only buttons next to the pinned-project toggle in Figma (an external link + a plain icon button, both resolving to a live Figma-prototype-server URL) — identified as Figma prototyping/dev-mode artifacts, not real design elements, so skipped.

## [2026-09-06] create | Work Experience profile-filling page flow (intro + list + Add/Edit/Delete/Success modals)

Built a standalone `/profile/filling/work` (+ `/profile/filling/work/history`) flow — distinct from the existing Career Buddy conversational Work Experience flow (`careerBuddyScript.js`, node range `5132:xxxxx`). This one uses Figma file key `Bin8roWL8sloyc36IgFMuT`, node range `5112`/`5113`/`5114`/`5178`/`5200`/`5249`/`6686`/`6286` — full detail in `wiki/figma-node-map.md` § "Work Experience — profile filling page flow".

**New files:**

- `src/components/sections/profileFilling/WorkIntroSection.jsx` + `src/pages/profileFilling/WorkIntroPage.jsx` — mirrors `InterestsIntroSection.jsx` structurally (breadcrumb, icon showcase, Impact/Time cards, 9-stage "Your journey" panel, 4 numbered info cards).
- `src/components/sections/profileFilling/WorkStage2Section.jsx` + `src/pages/profileFilling/WorkStage2Page.jsx` — the list page. Empty state ("No work history yet.") and populated state (role cards with laptop icon, status tag, description, skill tags, edit/delete icon buttons), a right aside ("What counts?" / "Why this matters" / "Recruiter views this week"), and a footer (Skills ← / → Portfolio).
- `src/components/sections/profileFilling/AddEditWorkModal.jsx` — single form component for both Add and Edit (via a `mode` prop), since Figma's two frames are the same form; Edit adds a "Delete this role" footer link + dirty-state "Unsaved changes" bar.
- `src/components/sections/profileFilling/DeleteWorkModal.jsx` — delete-confirm with role preview, two warning banners, and an "Edit instead?" nudge back to the Edit modal.
- `src/components/sections/profileFilling/WorkSavedModal.jsx` — success modal (trophy, role-count/years summary, "Senior role matching unlocked" checklist, mini role-list preview, profile-strength bar); wired to fire every 4th role saved (mirrors the frame's own "4 roles" framing — no backend to signal a real milestone in this mock).

**Modified files:**

- `src/App.jsx` — registered the two new routes alongside the other profile-filling routes.
- `src/components/sections/engagement/EngagementTopBar.jsx` — `STAGE_ROUTES['work-experience']` changed from the `/profile/engagement` placeholder to the real `/profile/filling/work` intro route, so the 9-stage trail's "Work" link now goes somewhere real (mirrors how `'personal-interests'` was already wired to `/profile/filling/interests`).

**Bug found + fixed post-build:** `buildRole()` in `WorkStage2Section.jsx` originally regenerated a role's `tags` array as `[employmentType]` on every save (add AND edit), silently discarding any skill/context tags (e.g. "React", "TypeScript", "Teaching") that have no matching field in Figma's own Add/Edit form. Reproduced via browser testing: editing the "ICT Instructor" role (tags `['National Service', 'Teaching']`) and saving with no changes dropped "Teaching". Fixed by having `buildRole()` swap only the employment-type entry within the existing tags array, preserving the rest.

**Verbatim-fidelity calls:** the intro page's 3rd info card ("Education tab" / schools-and-programmes copy) and the list page's header tag row ("3 categories / 4 specific interests / 4 min") are both unmistakable copy-paste leftovers from other stages in Figma's own source — the former kept verbatim (visible live), the latter replaced with real computed tags since displaying literal nonsense on a Work page would be actively misleading (documented in both figma-node-map.md and inline code comments). The Add-modal headline, suspected pre-build to be a copy-paste artifact reading "Identity captured.", was re-verified via `get_design_context` and confirmed to actually read "New role. Tell your story" / "Update your role." — the suspicion did not hold.

**Verified in browser:** full click-through at `/profile/filling/work` and `/profile/filling/work/history` — intro page renders (journey panel, info cards), list page empty→populated states, Add role (filled all fields, "I currently work here" checkbox correctly disables End Date, new card prepends to list, aside stats update), Edit role (pre-fills real values, tags now persist post-fix), Delete role (confirm dialog shows correct role preview, removes card, aside stats update), nested modal stacking (Delete-from-within-Edit) works correctly. No console errors. `npm run build` and `npm run lint` both clean (only the pre-existing unrelated `OnboardingRightPanel` lint warning).

**Not built / explicitly deferred:** a real UI for adding/removing the extra skill/context tags shown on cards (Figma's own form has no field for this); the vertical timeline-rail card treatment seen in some Figma sub-frames (the plainer card-stack treatment was used instead, screenshot-confirmed on the empty-state reference card).
